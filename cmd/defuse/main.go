...
func configureLogAddress(logger *slog.Logger, mgr *rcon.Manager, servers []store.Server) {
        ...
        // Optionally register an HTTP log sink (`logaddress_add_http <url>`), used by
        // CS2 builds that deliver logs over HTTP instead of (or alongside) UDP. This
        // is the full URL of the panel's HTTP log ingest endpoint as reachable from
        // the CS2 container, e.g. "http://defuse:8080/api/logs/http".
        httpSink := strings.TrimSpace(os.Getenv("CS2_LOG_HTTP_SINK_URL"))

        if sink == "" && httpSink == "" {
                logger.Info("CS2_LOG_SINK_ADDR and CS2_LOG_HTTP_SINK_URL unset; skipping RCON logaddress auto-config",
                        "hint", "set CS2_LOG_SINK_ADDR (e.g. defuse:27500) and/or CS2_LOG_HTTP_SINK_URL (e.g. http://defuse:8080/api/logs/http), or configure logaddress in the CS2 server yourself")
                return
        }
