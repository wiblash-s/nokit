...
//
// Newer CS2 servers also support HTTP log delivery via `logaddress_add_http
// <url>`, which POSTs log lines as a plain-text HTTP body instead of UDP
// datagrams. That transport is handled by the HTTP log listener (see
// internal/api), which parses the body with ParseHTTPBody and feeds each line
// into this same hub via Publish, so HTTP and UDP logs share one pipeline.
package loghub
...
// Publish broadcasts a pre-parsed log line to all current subscribers. It is
// used by alternative ingestion paths that receive log lines outside the UDP
// read loop — notably the HTTP log listener that accepts CS2
// `logaddress_add_http` payloads. Lines published here flow through the exact
// same fan-out as UDP lines, so they appear in the live SSE log view and drive
// the same workshop-map download verification. No-op if the hub is closed.
...
func ParsePacket(b []byte) (string, bool) {
        ...
        return cleanLine(string(body))
}

// cleanLine normalises a single raw Source log line: it trims trailing
// NUL/CR/LF bytes, drops the leading "L " marker, and trims surrounding
// whitespace. It returns ("", false) for empty/whitespace-only lines. Both the
// UDP packet parser and the HTTP body parser share this so lines look
// identical regardless of transport.
func cleanLine(s string) (string, bool) {
        line := strings.TrimRight(s, "\x00\r\n")
...
// ParseHTTPBody parses the body of a CS2 `logaddress_add_http` POST request into
// clean log lines. Unlike the UDP transport, HTTP delivery carries no 0xFFFFFFFF
// connectionless header or R/S type byte: CS2 simply POSTs the log content as a
// plain-text body containing one or more lines, each typically prefixed with the
// Source "L " marker and terminated by a newline. Each line is normalised with
// cleanLine; blank or unparseable lines are skipped. The returned lines are
// ready to hand to Hub.Publish.
func ParseHTTPBody(b []byte) []string {
        var lines []string
        for _, raw := range strings.Split(string(b), "\n") {
                if line, ok := cleanLine(raw); ok {
